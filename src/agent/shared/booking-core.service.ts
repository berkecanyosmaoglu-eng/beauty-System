...
type SessionState = {
  ...
  lastBookingSummary?: string;
  lastBookingStartAt?: string;
};
  ...
  private async handleBookingFlow(opts: {
    ...
    if (session.state === WaState.WAIT_CONFIRM) {
      ...
      if (session.editMode) {
        ...
        session.lastBookingSummary = reply;
        session.lastBookingStartAt = newIso;
        this.saveSession(key, session);
        return reply;
      }
      ...
      if (
        ...
        session.lastBookingSummary = reply;
        session.lastBookingStartAt = startAt;
        this.saveSession(key, session);
        return reply;
      }
      ...
      session.lastBookingSummary = successMsg;
      session.lastBookingStartAt = created.data.startAt;
      this.saveSession(key, session);
      return successMsg;
    }
